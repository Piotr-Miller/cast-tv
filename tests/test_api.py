...
def test_status_shape(app):
    ...
    assert isinstance(d["addresses"], list) and d["errors"] == 0
...
def test_cast_and_stop_over_api(app, tmp_path):
    ...
    _, _, s = _json(app.base_url, "GET", "/api/status")
    assert s["session"] == [{"source": "local", "id": str(clip), "name": "clip", "kind": "video",
...
def test_show_over_api(app, tmp_path):
    ...
    for name in ("a.jpg", "b.jpg", "c.jpg"):
        ...
        app.local.remember(item_for_path(str(p)))
        paths.append(str(p))
    status, _, d = _json(app.base_url, "POST", "/api/show",
                         {"items": [{"source": "local", "id": p} for p in paths], "interval": 5})
    assert status == 202
...
class _FakeSource:
    ...
    def thumb(self, source_id):
        if source_id == "none":
            return None
        return Upstream(self.base + "/" + source_id, headers={"Authorization": "Bearer t"})
